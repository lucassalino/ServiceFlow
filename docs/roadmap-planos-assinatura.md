...
- [x] **Histórico pessoal do voluntário** — nova secção "O meu histórico" em Definições → Perfil (`fetchMyServiceHistoryAction`), mostra total desde sempre, repartição por mês (barras) e os 10 eventos mais recentes onde serviu. Implementado em 2026-07-18. Plano Crescimento (a aplicar quando os planos forem implementados).

...
import {
  ...
  CalendarDays, Music2, Clock, Plus, CalendarOff, FileUp, AlertTriangle,
} from 'lucide-react';
...
function Sidebar({ current, onBack, onStepClick, step1Done }: {
  current: number; onBack: () => void;
    ...
    <div className="ep-sidebar" style={{
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '0.25rem' }}>
        {STEPS.map((s, i) => {
          ...
          const enabled = num === 1 || step1Done;
          return (
            <button
              key={i}
              onClick={() => enabled && onStepClick(num)}
              style={{
...
export function EventCreatePanel({ onBack }: Props) {
  const { activeOrg } = useOrgStore();
  const qc = useQueryClient();
  const { data: familyLinks } = useOrgFamilies(activeOrg?.id);
  ...
  const doSaveAll = handleSubmit(
    async (values) => {
      ...
      try {
        ...
        if (selectedSongIds.length > 0) await setupEventSetlistAction(ev.id, selectedSongIds, songKeys, songNotes);
                    ...
                    return (
                      ...
                      >
                        ...
                        <div style={{
                          ...
                          fontSize: '0.9rem', fontWeight: 800, color: m.color,
                        }}>
                        ...
                        <div key={ministryId} style={card}>
                          <div style={{
                            ...
                            <div style={{
                              ...
                              display: 'flex', alignItems: 'center', justifyContent: 'center',
                              background: `${ministry.color ?? 'var(--wis-blue)'}18`,

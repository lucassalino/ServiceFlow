            ...
            {[false, true].map((v) => (
              <button
                ...
                style={{
                  ...
                  background: annual === v ? 'var(--wis-blue)' : 'transparent',
                  color: annual === v ? '#fff' : 'var(--wis-text-2)',
                }}
              >
                {v ? 'Anual' : 'Mensal'}
              </button>
            ...
            return (
              <div
                ...
                style={{
                  ...
                  background: isCurrent ? 'var(--wis-blue-soft)' : 'var(--wis-surface-2)',
                  border: `1px solid ${isCurrent ? 'var(--wis-blue-border)' : 'var(--wis-border)'}`,
                }}
              >
                {isCurrent && (
                  <span style={{
                ...
                {/* Nome + preço */}
                <div>
                  <h3 style={{ fontSize: '1.05rem', fontWeight: 800, color: 'var(--wis-text)' }}>{plan.label}</h3>
                  <div style={{ display: 'flex', alignItems: 'baseline', gap: '0.3rem', marginTop: '0.15rem' }}>
                    <span style={{ fontSize: '1.35rem', fontWeight: 800, color: price === 0 ? 'var(--wis-success)' : 'var(--wis-text)' }}>
                      {fmtPrice(price)}
                    </span>
                    {price > 0 && (
                      <span style={{ fontSize: '0.72rem', color: 'var(--wis-text-3)' }}>/ {annual ? 'ano' : 'mês'}</span>
                    )}
                  </div>
                </div>
                ...
                <ul style={{ listStyle: 'none', padding: 0, margin: 0, display: 'flex', flexDirection: 'column', gap: '0.4rem' }}>
                  {plan.features.map((f) => (
                    <li key={f} style={{ display: 'flex', alignItems: 'flex-start', gap: '0.45rem', fontSize: '0.8rem', color: 'var(--wis-text)' }}>
                      <Check style={{ width: '0.85rem', height: '0.85rem', color: 'var(--wis-success)', flexShrink: 0, marginTop: '0.15rem' }} />
                      <span>{f}</span>
                    </li>
                  ))}
                </ul>
                ...
                <div style={{ marginTop: 'auto', paddingTop: '0.4rem' }}>
                  {isCurrent ? (
                    <button disabled style={{
                      width: '100%', padding: '0.55rem', borderRadius: '0.5rem', border: '1px solid var(--wis-border-strong)',
                      background: 'transparent', color: 'var(--wis-text-2)', fontWeight: 600, fontSize: '0.8rem', cursor: 'default',
                    }}>
                      O teu plano
                    </button>
                  ) : plan.priceMonthly === 0 ? (
                    ...
                    <button
                      ...
                      style={{
                        ...
                        background: 'var(--wis-blue-soft)', color: 'var(--wis-text)', fontWeight: 700, fontSize: '0.8rem',

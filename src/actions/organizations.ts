...
export async function leaveOrganizationAction(orgId: string): Promise<{ error?: string }> {
  const supabase = await createClient();
  const { data: { user }, error: authError } = await supabase.auth.getUser();
  ...
  if (adminsError) return { error: adminsError.message };

  const isOnlyAdmin = (admins ?? []).length === 1 && admins![0].user_id === user.id;
  if (isOnlyAdmin) {
    return {
      error: 'Não podes saír: és o único administrador desta organização. Atribui outro admin primeiro.',
    };
  }
